    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
      ...
      <div class="mt-4 md:mt-0 flex items-center gap-3">
        ...
        <router-link to="/artisan/products" 
          ...
          <svg class="w-5 h-5 rtl:ml-2 rtl:mr-0 ltr:mr-2 ltr:ml-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/></svg>
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <table v-else class="w-full text-left hidden md:table">
          ...
          <tbody class="divide-y divide-gray-100">
            <tr v-for="product in products" :key="product.id" @click="$router.push('/artisan/products/' + product.id)" class="hover:bg-gray-50 transition-colors cursor-pointer">
              ...
              <td class="px-6 py-4">
                <span class="px-3 py-1 rounded-full text-xs font-bold" :class="(product.stock || 0) < 5 ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'">{{ product.stock || 0 }}</span>
              </td>
              <td class="px-6 py-4 text-gray-600 font-semibold" dir="ltr">${{ Number(product.price || 0).toFixed(2) }}</td>
              <td class="px-6 py-4">
                <span v-if="product.has_ar" class="px-2 py-1 bg-purple-100 text-purple-700 text-xs font-bold rounded-md inline-flex items-center gap-1">AR</span>
              </td>
        ...
        <div class="md:hidden divide-y divide-gray-100">
          <router-link v-for="product in products" :key="product.id" :to="'/artisan/products/' + product.id" class="p-4 flex justify-between items-center block hover:bg-gray-50">
